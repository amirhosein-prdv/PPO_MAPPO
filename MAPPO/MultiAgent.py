    ...
    def _learn(self, agent_name: str) -> None:
        ...
        if self.logger is not None:
            ...
            self.logger.add_scalar(
                "charts/learning_rate", agent.policy.optimizer.param_groups[0]["lr"]
            )
            self.logger.add_scalar(
                f"losses/agent_{agent}/value_loss", critic_loss.item()
            )
            self.logger.add_scalar(
                f"losses/agent_{agent}/policy_loss", actor_loss.item()
            )
            self.logger.add_scalar(f"losses/agent_{agent}/entropy", entropy_loss.item())
            self.logger.add_scalar(f"losses/agent_{agent}/approx_kl", approx_kl.item())
            self.logger.add_scalar(f"losses/agent_{agent}/clipfrac", np.mean(clipfracs))
            self.logger.add_scalar(
                f"losses/agent_{agent}/explained_variance", explained_var
            )
